...
export class HighlightMaterial extends THREE.MeshBasicMaterial {
  onBeforeCompile(parameters: THREE.WebGLProgramParametersWithUniforms) {
    parameters.fragmentShader = parameters.fragmentShader.replace(
      '#include <clipping_planes_pars_fragment>',
      ['#include <clipping_planes_pars_fragment>', LINEAR_GRADIENT_GLSL].join(
        '\n'
      )
    );
