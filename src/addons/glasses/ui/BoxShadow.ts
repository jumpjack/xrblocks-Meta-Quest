...
export class BoxShadowMaterial extends THREE.MeshBasicMaterial {
  /**
   * Size of the BoxShadow.
   */
  size = new THREE.Vector2(180, 71);
  margin = new THREE.Vector2(15, 15);
  /**
   * Size of the box within the box shadow.
   */
  boxSize = new THREE.Vector2(146, 56);
  ...
  shadow1Color = new THREE.Vector4(0, 0, 0, 0.9);
  ...
  shadow2Color = new THREE.Vector4(0, 0, 0, 0);
...
`;
    ...
    parameters.fragmentShader = parameters.fragmentShader.replace(
      'void main() {',
      [shadowShader, 'void main() {'].join('\n')
    );

    parameters.fragmentShader = parameters.fragmentShader.replace(
      '#include <color_fragment>',
      ['#include <color_fragment>', 'diffuseColor = getShadowColor();'].join(
        '\n'
      )
    );
...
export class BoxShadow<
  OutProperties extends BoxShadowOutProperties = BoxShadowOutProperties,
> extends Custom<OutProperties> {
  name = 'Box Shadow';
